...
var cluster_nr = 2;				//Number of the cluster that contains the BrdU region (k-means method can sometimes switch cluster numbers).
var all_files = false;

var zone_width = 10;	//Width of the zones in microns
var fill_holes = false;
...
Dialog.addCheckbox("Fill holes in BrdU area?",fill_holes);
...
do {
	...
	for(i=0;i<nr_series;i++) {
		...
		for(z=0;z<count;z++) {
				...
				setResult("% area BrdU", z, area_perc);
